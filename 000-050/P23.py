...
f = open('abundant_nums.pkl', 'rb')
abundant_numbers = pickle.load(f)
f.close()

len_list = len(abundant_numbers)
sum_abundant_numbers = []
# Get all sum of abundant numbers less or equal than 28123
for index1 in range(len_list):
    for index2 in range(index1, len_list):
        sum12 = abundant_numbers[index1] + abundant_numbers[index2]
        if sum12 <= 28123:
            sum_abundant_numbers.append(sum12)
        else:
            break
             
with open('abundant_sums.pkl', 'wb') as f:
    pickle.dump(sum_abundant_numbers, f)
